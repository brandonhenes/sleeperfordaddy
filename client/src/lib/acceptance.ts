...
export function computeAcceptance(params: {
  ...
  if (fairness === "fair") { prob += 15; accept.push("Trade power is balanced"); }
  else if (fairness === "slight_edge" && delta > 0) { prob += 8; accept.push("Slight overpay in their favor"); }
  else if (fairness === "slight_edge" && delta <= 0) { prob -= 10; reject.push("You are getting the better end"); }
  else if (fairness === "lopsided") { prob -= 30; reject.push("Significantly unbalanced"); }
