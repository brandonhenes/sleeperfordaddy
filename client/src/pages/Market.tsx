...
import { useMarketSignals, type SignalType } from "../hooks/use-market-signals";
...
export default function Market() {
  ...
  return (
    <AppShell>
      ...
      {activeTab === "signals" && <SignalsTabContent username={effectiveUser} />}
    </AppShell>
  );
}
...
function SignalsTabContent({ username }: { username: string }) {
  ...
  if (isLoading) {
    return <div style={{ padding: 40, textAlign: "center", color: "var(--text-muted)" }}>Loading signals...</div>;
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
        {([
          ...
          >
            ...
            <div className="font-mono" style={{ fontSize: 22, fontWeight: 800, color: s.color }}>{s.count}</div>
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {signals.map((sig) => {
          const st = SIGNAL_STYLES[sig.signal] ?? SIGNAL_STYLES.CONSENSUS_LOCK;
          return (
            <div key={sig.player_id} style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 18px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                ...
                <span className="font-mono" style={{ background: st.bg, color: st.text, padding: "3px 10px", borderRadius: 6, fontSize: 11, fontWeight: 700 }}>{st.label}</span>
              </div>
              <div style={{ fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>{sig.reason}</div>
            </div>
          );
        })}
      </div>
