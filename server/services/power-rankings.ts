...
export async function getPowerRankings(username: string): Promise<LeaguePowerRanking[]> {
  ...
  const leagueRows = await db.execute(sql`
    SELECT league_id, name AS league_name, total_rosters,
           roster_positions, draft_rounds, previous_league_id
    FROM leagues WHERE league_id IN (${leagueInClause}) ORDER BY name ASC
  `);
  type LeagueRow = {
    league_id: string;
    league_name: string;
  ...
  for (const league of leagues) {
    ...
    const rosterPositions: string[] = (league.roster_positions as string[] | null) ?? DEFAULT_POS;
